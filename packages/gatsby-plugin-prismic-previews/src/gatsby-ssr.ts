/**
 * This file exports Gatsby's SSR APIs for this plugin.
 *
 * Prefer writing implementations in separate modules rather than poluting this file.
 *
 * APIs exported here must be re-exported manually in the root `gatsby-ssr.js`
 * file. Gatsby will not detect the exports if the module is re-exported as a whole.
 *
 * @see https://www.gatsbyjs.com/docs/reference/config-files/gatsby-ssr/
 */

export { onRenderBody } from "./on-render-body";
// TODO: Temporarily disabling automatic provider injection as it doesn't seem to work
// export { wrapRootElement } from './wrap-root-element'
